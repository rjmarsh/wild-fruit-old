...
function displayMatch(person) {
    var $display = $('<div>');
    $('<h3>').text(person.getDisplayName()).appendTo($display);
    $('<h3>').text("https://familysearch.org/platform/genealogies/persons/{pid}/matches").appendTo($display)
//    $("a").attr("href", "https://familysearch.org/ark:/61903/4:1:L5DC-H73?context=duplicates").appendTo($display)
    return $display;
}
